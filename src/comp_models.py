...
class ProductGuidanceLangevin(SDE):
    def __init__(
        ...
        if guidance_scales is None:
            self.guidance_scales = [0.0] * len(BasicMLP)
        else:
    ...
    def guided_score(self, x_t: torch.Tensor, t: torch.Tensor, y:  list[torch.Tensor] | None) -> torch.Tensor:
        ...
        guided_scores =  [self.guidance_scales[i] * (self.models[i](x_t,t,y[i]) - self.models[i](x_t, t, y_null)) for i in range(len(self.models))]
